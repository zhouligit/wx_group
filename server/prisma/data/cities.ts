/** 各省下辖地级行政区（地级市/盟/州等），空数组表示直辖市/港澳等本级建群 */
...
export function regionShortName(name: string): string {
  let s = name
    ...
    .replace('自治州', '');
  if (s.includes('族')) {
    return s.split('族')[0].slice(0, 2);
  }
  return s;
}

/** 每个城市至少 1 个群（主群） */
export const PRIMARY_GROUP = {
  suffix: '搭子交流群',
  desc: '同城搭子交友，加群一起玩',
  tag: '交友',
} as const;
...
/** 可选第二群（热门城市） */
export const SECONDARY_GROUP = {
  suffix: '饭搭子群',
  desc: '一起发现本地美食搭子',
  tag: '饭搭子',
} as const;

export const GROUP_TEMPLATES = [PRIMARY_GROUP, SECONDARY_GROUP] as const;
